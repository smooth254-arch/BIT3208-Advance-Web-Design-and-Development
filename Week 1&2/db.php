...
// If the users table already exists without a role field, add it now.
$result = $conn->query("SHOW COLUMNS FROM users LIKE 'role'");
if ($result && $result->num_rows === 0) {
    $conn->query("ALTER TABLE users ADD COLUMN role ENUM('admin','customer') NOT NULL DEFAULT 'customer' AFTER password");
}

$mainAdminEmail = 'smoothtech19@gmail.com';
$mainAdminPassword = 'Admin@123';
$mainAdminName = 'SmoothTech Admin';
$mainAdminPhone = '0000000000';
$checkAdmin = $conn->prepare("SELECT id FROM users WHERE email = ?");
$checkAdmin->bind_param('s', $mainAdminEmail);
$checkAdmin->execute();
$checkAdmin->store_result();
if ($checkAdmin->num_rows === 0) {
    $hash = password_hash($mainAdminPassword, PASSWORD_DEFAULT);
    $insertAdmin = $conn->prepare("INSERT INTO users (fullname, email, phone, password, role) VALUES (?, ?, ?, ?, 'admin')");
    $insertAdmin->bind_param('ssss', $mainAdminName, $mainAdminEmail, $mainAdminPhone, $hash);
    $insertAdmin->execute();
    $insertAdmin->close();
}
$checkAdmin->close();
